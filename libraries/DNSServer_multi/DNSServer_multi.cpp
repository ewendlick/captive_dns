...
DNSServer::DNSServer()
{
  ...
  // TODO: make this less hacky, make an array and check if the path exists in the array
  // TODO: handle requests that are http only?
  _googlePath1 = "clients3.google.com"; // /generate_204
  _googlePath2 = "www.gstatic.com"; // /generate_204
  _googlePath3 = "connectivitycheck.gstatic.com"; // /generate_204
  _iosPath = "captive.apple.com/hotspot-detect.html";
}

// TODO: pass in domain names that are blacklisted
...
void DNSServer::processNextRequest()
{
  ...
  if (packetSize >= sizeof(DNSHeader))
  {
    ...
    if (dnsHeader->QR == DNS_QR_QUERY &&
        ...
        (_domainName == "*" &&
         _domainName != _googlePath1 &&
         _domainName != _googlePath2 &&
         _domainName != _googlePath3 &&
         _domainName != _iosPath ||
            getDomainNameWithoutWwwPrefix(buffer, packetSize) == _domainName        )
       )
    {
